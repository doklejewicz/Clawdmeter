...
- **`tools/get_setup.sh` hang + corruption bugs fixed, `BOOTSTRAP.md` one-liner removed (2026-08-25).** The serial bootstrap client (see "Bootstrapping without the repo cloned" below) had two real bugs, both confirmed on real CYD hardware: `dd bs=1`'s byte-at-a-time reads were slow enough to lose bytes on their own (fixed → bulk `head -c`), and a missing timeout on the header-wait `read` meant a non-responding device hung the script forever rather than failing into the next retry — worse, a failed attempt's leftover trailing `SETUP_END` marker bytes stayed queued in the kernel's serial buffer across close/reopen and would desync the *next* attempt's header parse if not drained first. Retries bumped 3→6 since loss on this un-flow-controlled 115200-baud link is genuinely intermittent, not a single deterministic bug. Separately, `BOOTSTRAP.md`'s embedded one-liner (a from-scratch reimplementation of the same protocol for machines with no repo at all) was removed rather than fixed further — it had the same missing-timeout hang risk and was large enough to not be worth maintaining twice; the doc now just says to copy `tools/get_setup.sh` onto the target machine directly.
- **Live session/agent views merged onto `add-session-views` (branch, not yet on `main`).** Issue #135's feature — an opt-in Claude Code hooks sidecar (`daemon/clawdmeter_sessions.py`, full setup in `daemon/SESSIONS.md`) tracks every open session's status (waiting/working/idle), subagent/todo counts, and context-window fullness, ships it over a new BLE characteristic, and the firmware shows it as chat cards behind the `BOARD_HAS_SESSION_VIEWS` capability flag (only `waveshare_amoled_216` enables it today — needs real screen room for ~3 cards). Fixed on the way in: `elide_label()`'s head/tail math still assumed a 1-char ellipsis after a later commit widened it to ASCII `"..."`, so every elided label came out 2 chars too long; and `ui.cpp`'s `update_view_state()` needed hand-merging with `main`'s newer `data_ok` idle-gate fix rather than a mechanical conflict resolution — see that function's comments for how the two interact.
...
**Bootstrapping without the repo cloned:** a flashed device with the setup
bundle on its SPIFFS partition (built via `tools/build_setup_bundle.sh`,
flashed via `pio run -t uploadfs` — a separate step from the normal firmware
upload) will hand `install.sh` and everything it needs to a bare machine over
serial in response to a `get-setup` command (`send_setup_bundle()` in
`main.cpp`). `tools/get_setup.sh` is the client: puts the port in raw mode,
verifies the transfer against the device's own MD5, retries up to 6 times.
This link is genuinely lossy at 115200 baud with no flow control — a
byte-at-a-time `dd bs=1` read measurably made it worse (confirmed on
hardware), so the payload read uses a single bulk `head -c` instead, plus a
timeout on the header wait so a non-responding attempt can't hang forever.
Full writeup: `daemon/BOOTSTRAP.md`.
