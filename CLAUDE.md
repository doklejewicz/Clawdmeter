...
- **Unverified on real hardware**: `LCD_ROTATION` (board.h, currently `1`) and the touch axis mapping are a best guess from reading the GFX/XPT2046 library source, not confirmed against a physical unit — flip to `LCD_ROTATION 3` if the image is sideways/mirrored on first flash. BLE advertising is also unverified — this is the first classic-ESP32 NimBLE-Arduino build in this repo.
